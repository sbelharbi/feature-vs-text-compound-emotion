...
class Postprocessor(nn.Module):
    ...
    embeddings in the same format for a large set of YouTube videos. This class
    implements the same PCA (with whitening) and quantization transformations."
    """
    ...
    def __init__(self):
        ...
        self.pca_eigen_vectors = nn.Parameter(self.pca_eigen_vectors,
                                              requires_grad=False)
    ...
    def postprocess(self, embeddings_batch):
        ...
        pca_applied = torch.mm(self.pca_eigen_vectors,
                               (embeddings_batch.t() - self.pca_means)).t()
...
class VGGish(VGG):
    def __init__(self, urls, device=None, pretrained=True, preprocess=True,
                 postprocess=True, progress=True):
    ...
    def forward(self, x, fs=None):
        ...
        x = torch.tensor(x)[:, None, :, :].float().to(self.device)
    ...
    def _preprocess(self, x, fs):
        raise ValueError("The preprocess should be executed before the call of VGGish model!")
