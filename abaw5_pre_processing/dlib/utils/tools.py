...
from abaw5_pre_processing.dlib.utils.shared import fmsg
# from abaw5_pre_processing.project.abaw5 import _constants
from abaw5_pre_processing.dlib.utils.shared import is_cc
from abaw5_pre_processing.dlib.utils.shared import get_tag_device
...
def better_chunks_into_n(l: Iterable, n: int) -> Iterable:
    idx = list(range(len(l)))
    np_idx = np.array_split(idx, n)
    assert len(np_idx) == n, f"{len(np_idx)} | {n}"
    out = []
    for i in range(n):
        chunk = np_idx[i]
        h = []
